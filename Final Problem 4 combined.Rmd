...
**Our backward selection recommends us to remove all interactions and use our previous model.**
...
**As we can see, the AIC for Model 2 (including *log(yrs.since.phd)*) is marginally better, so we will leave this in the model.**  
...
```{r}
SalM5<- lm(1/salary~rank+discipline+log(yrs.since.phd)+yrs.service, data= Salaries)
AIC(SalM2,SalM5)
```
We always will look for smaller values of AIC (lowest sum of squares least complexity). Model 2 has a slightly smaller AIC score and is probably better. We will conduct a 5 Fold Cross-Validation in part 2, to compare Model 5 and Model 2.
$Model\ 2:\ 1/salary:rank+discipline+log(yrs.since.phd)+yrs.service+sex$
$Model\ 5:\ 1/salary:rank+discipline+log(yrs.since.phd)+yrs.service$
...
We will compare Model 2 and Model 5 with the 5 Fold Cross-Validation.
...
k=foreach(i=1:5)%do%
  {
    sets=which(id==i)
      # remove set
      Mod1<- lm(1/salary~rank+discipline+log(yrs.since.phd)+yrs.service+sex, data= Salaries[-sets,])
      Mod2<- lm(1/salary~rank+discipline+log(yrs.since.phd)+yrs.service,data=Salaries[-sets,])
...
**Applying 5-fold cross validation to compare these models shows there is not an improvement by adding sex as a predictor. Therefore, the simpler model excluding sex should be used and a gender based pay gap may not exist. Given that Model 5 happens to be a sub-model of Model 2, we can also compare them via ANOVA:**

```{r}
anova(SalM5,SalM2)
```

**With $p-value > 0.05$, we fail to reject $H_0$ => conclude the models are the same => use the less complex *Model 5* excluding sex as a predictor. This confirms the previous assertion that a gender based pay gap does not exist.**
...
plot(SalM5)
summary(SalM5)
plot(allEffects(SalM5))
```

**Through the exploration above, it appears the best model is Model 5 with rank, discipline, years since PhD, and Years of Service as the predictors. Given this information about a potential hire, which is legal to collect and use, we could certainly predict an appropriate range for an offer of employment.**   

The residual plot shows constant variance.  
The Normal Q-Q plot shows normality, the data points fall approximately along with the reference line.  
The Scale-Location plot does not show a significant trend.  
The last plot shows no outliers as the cook's distance lines are not visible.  
...
predictPI<- predict(SalM5, newEmployee, interval="prediction", level=0.95)
predictPI<-1/predictPI
CIPred<- predict(SalM5, newEmployee, interval="confidence", level=0.95)
CIPred<-1/CIPred
rbind(predictPI,CIPred)
```

The 95%-prediction interval indicates that the average log nine-month salary will be between `r round(predictPI[3], digits = 0)` and `r round(predictPI[2], digits = 0)` while the 95 CI shows the salary to be between `r round(CIPred[3], digits = 0)` and `r round(CIPred[2],, digits = 0)`.

**What if we used Model 2 which includes sex as a predictor?** 
```{r}
attach(Salaries)
EmpMale= data.frame(sex="Male", rank="AsstProf", discipline="A", yrs.service=1, yrs.since.phd=4)
EmpFem=data.frame(sex="Female", rank="AsstProf", discipline="A", yrs.service=1, yrs.since.phd=4)

PIM<- predict(SalM2, EmpMale, interval="prediction", level=0.95)
PIM<-1/PIM
CIM<- predict(SalM2, EmpMale, interval="confidence", level=0.95)
CIM<-1/CIM
rbind(PIM,CIM)

PIF<- predict(SalM2, EmpFem, interval="prediction", level=0.95)
PIF<-1/PIF
CIF<- predict(SalM2, EmpFem, interval="confidence", level=0.95)
CIF<-1/CIF
rbind(PIF,CIF)

PIM[1]-PIF[1] #Male Prediction - Female Prediction
```
